...
final class TrinoSystemRequirements
{
    ...
    private static void verifyOsArchitecture()
    {
        ...
        if ("Linux".equals(osName)) {
            if (!ImmutableSet.of("amd64", "aarch64", "ppc64le", "loongarch64", "loong64").contains(osArch)) {
                failRequirement("Trino requires amd64, aarch64, ppc64le or loongarch64 on Linux (found %s)", osArch);
            }
            ...
            else if ("loongarch64".equals(osArch) || "loong64".equals(osArch)) {
                warnRequirement("Support for the POWER architecture is experimental");
            }
        }
